			    ...
			    struct ptlrpc_thread *thread)
{
	...
	if (ktime_get_real_seconds() - req->rq_arrival_time.tv_sec > 5)
		DEBUG_REQ(D_WARNING, req, "Slow req_in handling "CFS_DURATION_T"s",
			  (long)(ktime_get_real_seconds() - req->rq_arrival_time.tv_sec));
			     ...
			     struct ptlrpc_thread *thread)
{
	...
	if (likely(request->rq_export)) {
		if (unlikely(ptlrpc_check_req(request)))
			goto put_conn;
		ptlrpc_update_export_timer(request->rq_export, timediff_usecs >> 19);
	}
	...
	if (ktime_get_real_seconds() > request->rq_deadline) {
		DEBUG_REQ(D_ERROR, request, "Dropping timed-out request from %s: deadline " CFS_DURATION_T ":" CFS_DURATION_T "s ago\n",
			  ...
			  (long)(request->rq_deadline - request->rq_arrival_time.tv_sec),
			  (long)(ktime_get_real_seconds() - request->rq_deadline));
		goto put_conn;
	}
	...
	if (unlikely(ktime_get_real_seconds() > request->rq_deadline)) {
		DEBUG_REQ(D_WARNING, request,
			  "Request took longer than estimated "
				"(%lld:%llds); client may timeout.",
			  (s64)request->rq_deadline - request->rq_arrival_time.tv_sec,
