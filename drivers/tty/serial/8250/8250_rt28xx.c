...
static int serial8250_rt28xx_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(np, "reg-offset", &prop) == 0)
		up.port.mapbase += prop;

	/* Check for registers offset within the devices address range */
	if (of_property_read_u32(np, "reg-shift", &prop) == 0)
		up.port.regshift = prop;

	/* Check for fifo size */
	if (of_property_read_u32(np, "fifo-size", &prop) == 0)
		up.port.fifosize = prop;
...
static int serial8250_rt28xx_remove(struct platform_device *pdev)
{
...
static const struct of_device_id serial8250_rt28xx_dt_ids[] = {
	{ .compatible = "ralink,rt2880-uart", },

	{},
};
MODULE_DEVICE_TABLE(of, serial8250_rt28xx_dt_ids);

static struct platform_driver serial8250_rt28xx_platform_driver = {
	.driver = {
