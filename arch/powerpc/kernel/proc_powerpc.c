...
{
	struct inode *inode = file->f_path.dentry->d_inode;
	return simple_read_from_buffer(buf, nbytes, ppos, inode->i_private,
				       inode->i_size);
}
...
static int page_map_mmap( struct file *file, struct vm_area_struct *vma )
{
	...
	remap_pfn_range(vma, vma->vm_start, __pa(inode->i_private) >> PAGE_SHIFT,
						inode->i_size, vma->vm_page_prot);
	return 0;
}
