				 ...
				 unsigned end_line)
{
	...
	if (unlikely(timeit)) {
		kt_end = ktime_get();
		if (ktime_to_ns(par->update_time) == 0) {
			par->update_time = kt_end;
		}
		par->update_time = kt_start;
		fps_us = ktime_us_delta(kt_start, par->update_time);
		fps = fps ? 1000000 / fps : 0;

		duration_us = ktime_us_delta(kt_end, kt_start);
		throughput = duration_us ? (len * 1000) / duration_us : 0;
		throughput = throughput * 1000 / 1024;

		dev_info(par->info->device,
			"Display update: %ld kB/s (%ld.%.3ld ms), fps=%ld (%ld.%.3ld ms)\n",
			throughput, duration_us / 1000, duration_us % 1000,
			fps, fps_us / 1000, fps_us % 1000);
