...
{
	gpiod_set_value(wl->power_gpio, enable);
...
static int wl1251_spi_probe(struct spi_device *spi)
{
	...
	wl->use_eeprom = device_property_read_bool(&spi->dev,
						   "ti,wl1251-has-eeprom");
	wl->power_gpio = devm_gpiod_get(&spi->dev, "ti,power", GPIOD_OUT_LOW);
	if (IS_ERR(wl->power_gpio)) {
		ret = PTR_ERR(wl->power_gpio);
		if (ret != -EPROBE_DEFER)
