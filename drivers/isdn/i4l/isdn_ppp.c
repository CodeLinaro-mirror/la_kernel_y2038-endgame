...
static int isdn_ppp_gidle(int min, struct file *file, unsigned int cmd,
...
static int isdn_ppp_scompress(int min, struct file *file, unsigned int cmd,
...
isdn_ppp_compat_ioctl(int min, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case PPPIOCGDEBUG:
	case PPPIOCSDEBUG:
		return isdn_ppp_ioctl(min, file, cmd, argp);
	case PPPIOCGIDLE32:
		return ppp_gidle(min, file, cmd, argp);
	case PPPIOCSCOMPRESS32:
		return ppp_scompress(min, file, cmd, argp);
	case PPPIOCSPASS32:
	case PPPIOCSACTIVE32:
		return ppp_sock_fprog_ioctl_trans(min, file, cmd, argp);
	}
