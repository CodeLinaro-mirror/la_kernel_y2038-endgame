...
{
	diva_os_xdi_adapter_t *a = file->f_path.dentry->d_inode->i_private;
...
{
	return single_open(file, d_l1_down_proc_show, inode->i_private);
}
...
{
	return single_open(file, grp_opt_proc_show, inode->i_private);
}
...
{
	return single_open(file, info_proc_show, inode->i_private);
}
