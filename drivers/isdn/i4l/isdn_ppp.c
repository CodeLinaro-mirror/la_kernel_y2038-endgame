...
static int isdn_ppp_sock_fprog_ioctl_trans(int min, struct file *file, unsigned int cmd,
					   struct sock_fprog32 __user *u_fprog32)
{
	...
	return isdn_ppp_ioctl(min, file, cmd, (unsigned long) u_fprog64);
}
...
static int isdn_ppp_compat_gidle(int min, struct file *file, unsigned int cmd,
				 struct ppp_idle32 __user *idle32)
{
	...
	err = isdn_ppp_ioctl(min, file, PPPIOCGIDLE, (unsigned long) idle);
...
static int isdn_ppp_compat_scompress(int min, struct file *file,
				     unsigned int cmd,
				     struct ppp_option_data32 __user *odata32)
{
	...
	return isdn_ppp_ioctl(min, file, PPPIOCSCOMPRESS, (unsigned long) odata);
}

/*
 * this handler implements whatever the generic conversion handler
 * in fs/compat_ioctl.c used to do, just moved into i4l.
 * If anyone wants to really use this in compat mode, they probably
 * need to add handlers for the other commands as well.
 */
int
isdn_ppp_compat_ioctl(int min, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case PPPIOCGDEBUG:
	case PPPIOCSDEBUG:
		return isdn_ppp_ioctl(min, file, cmd, arg);
	case PPPIOCGIDLE32:
		return isdn_ppp_compat_gidle(min, file, cmd, argp);
	case PPPIOCSCOMPRESS32:
		return isdn_ppp_compat_scompress(min, file, cmd, argp);
	case PPPIOCSPASS32:
	case PPPIOCSACTIVE32:
		return isdn_ppp_sock_fprog_ioctl_trans(min, file, cmd, argp);
	}
